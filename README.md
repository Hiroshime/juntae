...
docker buildx build \
  ...
  --tag docker.io/SEU_USUARIO/juntae:0.10.0 \
...
Depois abra **Actions → Publicar imagem Docker → Run workflow**, informe `0.10.0` e execute. O workflow
publicará `SEU_USUARIO/juntae:0.10.0` e `SEU_USUARIO/juntae:latest`. Fazer push de uma tag Git como
`v0.10.0` também publica automaticamente as tags `0.10.0` e `latest`.
...
Para atualizar, publique uma nova versão imutável, como `0.10.0`, altere `JUNTAE_IMAGE`, gere novamente
