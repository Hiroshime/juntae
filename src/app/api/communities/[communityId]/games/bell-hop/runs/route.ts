...
import { startBellHopRun } from "@/server/services/bell-hop-service";
...
export async function POST(
  ...
  try {
    ...
    await enforceRateLimit(
      `bell-hop-start:${ids.data.communityId}:${user.id}`,
      40,
      60 * 60 * 1_000,
    );
    return NextResponse.json(
      { run: await startBellHopRun(user.id, ids.data.communityId) },
      { status: 201 },
    );
  } catch (error) {
